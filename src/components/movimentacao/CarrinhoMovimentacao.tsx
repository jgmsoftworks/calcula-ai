...
export function CarrinhoMovimentacao({
  carrinho,
  onCarrinhoChange
}: any) {
  const {
  ...
  useEffect(() => {
    ...
    const loadFuncionarios = async () => {
      const {
        data
      } = await supabase.from('folha_pagamento').select('id, nome').eq('user_id', user.id).eq('ativo', true).order('nome');
      setFuncionarios(data || []);
    };
    loadFuncionarios();

    // Realtime - sincronizar mudanças na folha de pagamento
    const channel = supabase.channel('folha-realtime').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'folha_pagamento',
      filter: `user_id=eq.${user.id}`
    }, () => loadFuncionarios()).subscribe();
    return () => {
  ...
  return <Card className="p-4 flex flex-col h-[545px]">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {totalItens > 0 && <span className="ml-auto text-sm text-muted-foreground">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {carrinho.length === 0 ? <div className="text-center text-muted-foreground py-8 text-sm">
            ...
            {carrinho.map((item: any, index: number) => <ItemCarrinho key={`${item.produto_id}-${index}`} item={item} onRemove={() => handleRemoveItem(index)} />)}
          </div>}
      </div>
      ...
      <div className="flex-shrink-0 space-y-4 mt-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={responsavel} onValueChange={setResponsavel}>
            ...
            <SelectContent>
              {funcionarios.map(func => <SelectItem key={func.id} value={func.nome}>
                  {func.nome}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2">
          ...
          <Textarea value={observacao} onChange={e => setObservacao(e.target.value)} placeholder="Observações sobre a movimentação..." rows={2} />
        </div>

        <Button onClick={handleFinalizar} disabled={carrinho.length === 0 || !responsavel || finalizing} className="w-full">
